    ...
    public class CosmicTime : ISerializable
    {
        ...
        public static DateTime ToDateTime(Duration duration)
        {
            ...
            var ticks = ((duration.Years * (ulong)Duration.NanosecondsPerYear)
                ...
                + (ulong)Math.Round((duration.TotalYoctoseconds + (duration.PlanckTime / Duration.PlanckTimePerYoctosecond)) / Duration.YoctosecondsPerNanosecond))
        ...
        /// </para>
        /// <para>
        /// Adding infinite time is treated as adding none. Adding an amount which would cause the
        /// result to overflow to infinity causes the result to be set to the maximum allowed value
        /// instead.
        /// </para>
        /// </param>
